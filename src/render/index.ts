...
function renderExpanded(ctx: RenderContext, terminalWidth: number | null = null): Array<{ line: string; isActivity: boolean }> {
  ...
  for (let index = 0; index < elementOrder.length; index += 1) {
    ...
    if (element === 'project' && nextElement === 'context' && !seen.has('context')) {
      seen.add(element);
      seen.add(nextElement);

      const projectLine = renderElementLine(ctx, element);
      const contextLine = renderElementLine(ctx, nextElement);

      if (projectLine && contextLine) {
        lines.push({ line: `${projectLine} │ 🧠 ${contextLine}`, isActivity: false });
      } else if (projectLine) {
        lines.push({ line: projectLine, isActivity: false });
      } else if (contextLine) {
        lines.push({ line: contextLine, isActivity: false });
      }
    ...
    // Merge environment with the first adjacent activity line (tools/agents/todos)
    if (element === 'environment' && ACTIVITY_ELEMENTS.has(nextElement) && !seen.has(nextElement)) {
      seen.add(element);
      seen.add(nextElement);

      const envLine = renderElementLine(ctx, element);
      const actLine = renderElementLine(ctx, nextElement);

      if (envLine && actLine) {
        lines.push({ line: `${envLine}  │  ${actLine}`, isActivity: false });
      } else if (envLine) {
        lines.push({ line: envLine, isActivity: false });
      } else if (actLine) {
        lines.push({ line: actLine, isActivity: true });
      }
